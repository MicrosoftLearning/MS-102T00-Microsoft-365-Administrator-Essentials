...
10. Note that Microsoft selected the **Directory roles** checkbox. Select the field below this option. In the menu of roles that you can choose to exclude from MFA, the only role that Microsoft selected is **Directory Synchronization Accounts**. This role is automatically assigned to the Microsoft Entra Connect Sync service and is not meant to be used otherwise. It's a special role that has permissions only to perform directory synchronization tasks. This role is crucial for synchronizing a local Active Directory Domain Services (AD DS) to Microsoft Entra ID, providing identity coexistence. Active Directory and Microsoft Entra ID. These accounts are typically configured with a long complex password that doesn't expire, and they're granted a special role that has only permissions to perform directory synchronization tasks. <br/>

	Microsoft doesn't require MFA for this role because the synchronization service needs to run continuously without any interruptions that could be caused by MFA prompts. Additionally, these accounts are usually configured with very limited permissions and aren't used for interactive sign-ins, which reduces the security risk.   <br/>
