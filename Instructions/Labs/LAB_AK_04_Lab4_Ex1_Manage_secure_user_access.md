...
**Impact on this lab exercise:** So what this all means is that you will still create your Conditional Access policy as originally designed in Task 1. The policy has not changed from its original design - it requires MFA for all users, except for the members of the Microsoft 365 pilot project group. However, since the policy that Microsoft built into the lab's trial tenant is the more restrictive of the two policies, that policy takes precedence during enforcement. That means you must use MFA when signing in as each test user through the remainder of these labs. As a result, we have removed the original Task 2 that tested the Conditional Access policy that you created, since Microsoft Entra won't apply it in this trial tenant. But even though you can't test your policy, we still want you to go through the experience of creating it in Task 1 so that you can apply that knowledge in your real-world deployments. 
